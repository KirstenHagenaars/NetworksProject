...
start_client = "python3 client_app.py".format(intf)
start_server = "python3 server_app.py".format(intf)
...
class TestbTCPFramework(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.server = None
        try:
            self.server = subprocess.Popen(start_server, shell=True, cwd=None, stdout=subprocess.PIPE,
                                           stdin=subprocess.PIPE)
        except Exception as inst:
            print("Server crashed: ", str(inst))
    ...
    def test_ideal_network(self):
        ...
        # launch localhost client connecting to server & client sends content to server
        run_command(start_client)

        # server receives content from client
        self.server.communicate()

        # content received by server matches the content sent by client
        self.assertTrue(filecmp.cmp("input.file", "output.file", shallow=False))
